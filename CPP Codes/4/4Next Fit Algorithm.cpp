...
void nextFit(vector<MemoryBlock>& memory, vector<Process>& processes) {
    cout << "\n=== NEXT FIT MEMORY ALLOCATION ===" << endl;
    ...
    for(auto& process : processes) {
        ...
        do {
            if(!memory[currentIndex].allocated && memory[currentIndex].size >= process.size) {
                // Allocate this block to the process
                memory[currentIndex].allocated = true;
                memory[currentIndex].processId = process.id;
                memory[currentIndex].processName = process.name;
                
                process.allocated = true;
                process.blockId = memory[currentIndex].id;
                lastAllocatedIndex = (currentIndex + 1) % memory.size();
                
                cout << "Process " << process.name << " (" << process.size 
                     << "KB) allocated to Block " << memory[currentIndex].id 
                     << " (" << memory[currentIndex].size << "KB)" << endl;
                cout << "Internal Fragmentation: " << (memory[currentIndex].size - process.size) << "KB" << endl;
                
                allocated = true;
                break;
            }
            
            currentIndex = (currentIndex + 1) % memory.size();
