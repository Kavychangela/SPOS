...
void worstFit(vector<MemoryBlock>& memory, vector<Process>& processes) {
    ...
    for(auto& process : processes) {
        ...
        // Find the worst fit block (largest block that can hold the process)
        ...
        if(worstBlockIndex != -1) {
            ...
            memory[worstBlockIndex].allocated = true;
            memory[worstBlockIndex].processId = process.id;
            memory[worstBlockIndex].processName = process.name;
            
            process.allocated = true;
            process.blockId = memory[worstBlockIndex].id;
            
            cout << "Process " << process.name << " (" << process.size 
                 << "KB) allocated to Block " << memory[worstBlockIndex].id 
                 << " (" << memory[worstBlockIndex].size << "KB)" << endl;
            cout << "Internal Fragmentation: " << maxFragmentation << "KB" << endl;
        } else {
            cout << "Process " << process.name << " (" << process.size 
